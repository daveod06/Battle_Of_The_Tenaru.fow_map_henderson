...
_spawnDir=(_mPos getDir _spawnPos);
    ...
    if (ismultiplayer) then {
            ...
            if (_heightLimit) then 
                ...
                //_actCond      = "{(vehicle _x in thisList) && (side _x isEqualTo GUER) && (((getPosATL _x) select 2) < 5)} count allUnits > 0";
                _actCond      = "true";
...
//SPAWN LIGHT VEHICLES      
    ...
    for "_counter" from 1 to _LVehGroups do {
        ...
        _newpos = [_mPos, (_Placement +300), _spawnDir + (random [-_spawnRandomDir,0,_spawnRandomDir])] call BIS_fnc_relPos;
